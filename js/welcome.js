...
//     data: {
//         key: '2EGBZ-XUZYI-XDQG2-U6HFO-C54FQ-OEBVB',
//         output: 'jsonp',
//     },
//     dataType: 'jsonp',
...
//get请求 - 使用ip-api.com API（原api.qjqq.cn已失效）
...
function showDefaultWelcome() {
    ...
    try {
        document.getElementById("welcome-info").innerHTML =
            `<b>欢迎来到 YYSの小窝</b>💖<br>很高兴遇见你！🍂<br>${timeChange}<br>`;
    } catch (err) {
        console.log("Pjax无法获取元素");
    }
}
...
function showWelcome() {
    // 检查API返回的数据是否有效
    if (!ipLocation || ipLocation.status !== 'success') {
    ...
    // ip-api.com 返回的数据结构：
    // country, regionName(省份), city, lat, lon, query(IP)
    let dist = getDistance(121.413921, 31.089290, ipLocation.lon, ipLocation.lat); //修改自己的经度（121.413921）纬度（31.089290）
